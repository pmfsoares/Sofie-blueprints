...
const devices: DeviceEntry[] = [
	...
	},
	{
		id: 'TX',
		firstVersion: '0.1.0',
		type: TSR.DeviceType.QUANTEL,
		defaultValue: (input): TSR.DeviceOptionsQuantel => ({
			type: TSR.DeviceType.QUANTEL,
			options: literal<TSR.QuantelOptions>({
				gatewayUrl: input.gatewayUrl,
				ISAUrlMaster: input.ISAUrlMaster,
				serverId: 1100,
			}),
		}),
		input: [
	...
	},
	{
		...
		defaultValue: (input): TSR.DeviceOptionsOBS => ({
			type: TSR.DeviceType.OBS,
			options: literal<TSR.OBSOptions>({
				host: input.host,
				port: input.port,
